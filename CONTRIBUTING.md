...
2. Prefix every skill name with `ei-` so Edge Impulse skills are identifiable once installed alongside skills from other catalogs. After the prefix, use an action-oriented segment: `ei-firmware-` for skills that integrate exported Edge Impulse models into device or application code and `ei-build-` for build and integration workflows; use the exact name `ei-api` for Edge Impulse platform integrations. Follow that with the relevant platform, tool, or language, such as `ei-build-arduino-` or `ei-firmware-stm32`. Spell product names as kebab-case components, such as `uno-q`, `app-lab`, and `raspberry-pi`; add `-cpp` or `-python` when the language distinguishes the workflow.
3. The `name` field in `SKILL.md` frontmatter must match the skill's directory name exactly.
4. Add or update the skill's entry in the catalog table in `README.md`.
